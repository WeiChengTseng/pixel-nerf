    ...
    def forward(self, alpha_fine):
        if self.lambda_alpha > 0.0 and self.epoch.item() >= self.init_epoch:
            ...
            if self.force_opaque:
                ...
                alpha_loss = torch.log(alpha_fine) + torch.log(1.0 -
                                                               alpha_fine)
...
def get_alpha_loss(conf):
    ...
    return AlphaLossNV2(lambda_alpha,
                        clamp_alpha,
                        init_epoch,
...
class RGBWithUncertainty(torch.nn.Module):
    ...
    def __init__(self, conf):
        super().__init__()
        self.element_loss = (torch.nn.L1Loss(
            reduction="none") if conf.get_bool("use_l1") else torch.nn.MSELoss(
                reduction="none"))
...
class RGBWithBackground(torch.nn.Module):
    ...
    def forward(self, outputs, targets, lambda_bg):
        ...
        weighted_element_err = torch.mean(self.element_loss(outputs, targets),
                                          -1) / (1 + lambda_bg)
        return torch.mean(weighted_element_err) + torch.mean(
            torch.log(lambda_bg))
...
def get_rgb_loss(conf, coarse=True, using_bg=False, reduction="mean"):
    ...
    return (torch.nn.L1Loss(reduction=reduction) if conf.get_bool("use_l1")
